...
class BulkCryptoDetailsSerializer(serializers.Serializer):
    ...
    def save(self, *kwargs):
        ...
        for item in self.validated_data['data']:
            ...
            if crypto_name:
                try:
                    instance = crypto_tracker_models.CryptoDetail.objects.get(name=crypto_name)
        ...
        if bulk_update_list:
            crypto_tracker_models.CryptoDetail.objects.bulk_update(
                bulk_update_list, 
                fields=[
                    'price', 'one_hour_change', 'one_day_change', 'one_week_change', 'market_cap', 'volume_in_dollars', 'volume_in_crypto', 'circulating_supply', 'updated_at'
                ]
            )
        if bulk_create_list:
